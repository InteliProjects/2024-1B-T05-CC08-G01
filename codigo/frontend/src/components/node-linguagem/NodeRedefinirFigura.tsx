...
function NodeRedefinirFigura(props: NodeProps) {
  const data = props.data || { ref: 0, tipo: "", cor: "", x: 0, y: 0, tamanho: 0, code: "" };
  ...
  const handleInputChange = (event: any, setter: Function, key: string) => {
    ...
    if (props.data && props.data.onUpdate) {
      props.data.onUpdate({ ...props.data, [key]: value, code: `redefinir_figura(${ref}, "${tipo}", "${cor}", ${x}, ${y}, ${tamanho});` });
    }
  };
...
export default NodeRedefinirFigura;
