...
class AnalisadorSintatico:
	...
	def comparar(self, tipoEsperado):
		...
		if self.tokenCorrente.tipo == tipoEsperado.upper():
			...
			print(f'Erro: {self.tokenCorrente.tipo} {self.tokenCorrente.valor} {tipoEsperado}')
	...
	def assign_statement(self):
		...
		if self.tokenCorrente.tipo in ['FUNCIN']:
			...
			string = self.str()
			self.comparar('SEMICOLON')
			return NoInterno(op='assignStatement', id=NoFolha(op='id', valor=id.valor, linha=id.linha), string=string)
		else:
	...
	def factor(self):
		...
		if self.tokenCorrente.tipo == 'ID':
			...
			return NoInterno(op='factor', dir=None, esq=None, factor=NoFolha(op='bool', valor=factor.valor, linha=factor.linha), sinal="+")
...
if __name__ == '__main__':
  import sys
  import os
# with open("./exemplos_teste/ex2_s_erro_codigo.w", 'r', encoding='utf-8') as arquivo:
...
tokens = [
 Token('PROGRAMA', 'programa', '1'), 
 Token('DQUOTE', '"', '2'), 
 Token('STR', 'Programa com variável numero que recebe texto', '2'), 
 Token('DQUOTE', '"', '2'), 
 Token('VAR', 'var', '3'), 
 Token('TYPE', 'numero', '4'), 
 Token('ID', 'x', '4'), 
